...
describe(`User story: Answer feedback`, function() {
  ...
  context(`Given I submit my answer`, () => {
    ...
    it(`submits my answer typed in the form`, () => {
      ...
      cy.get('main form').within($form => {
        cy.get('input#guess-word').type(guess);
  ...
  context(`Given guess is incorrect`, () => {
    ...
    it(`displays score and feedback the word was incorrect`, () => {
      ...
      const fixtures = [];
      Cypress.Promise.all([
        cy.fixture('language-head.json').then(langHeadFx => fixtures.push(langHeadFx)),
        cy.fixture('language-guess-incorrect.json').then(langGuessIncFx => fixtures.push(langGuessIncFx))
      ]).then(() => {
        const [languageHeadFixture, incorrectFixture] = fixtures;

        cy.get('main').within($main => {
          cy.get('.Score__text').should('have.text', `Your total score is: ${incorrectFixture.totalScore}`);
          cy.get('.feedback').should('contain', `Good try, but not quite right`);
  ...
  context(`Given guess is correct`, () => {
    ...
    it(`gives feedback the word was correct`, () => {
      ...
      Cypress.Promise.all([
        ...
        cy.get('main').within($main => {
          cy.get('.Score__text').should('have.text', `Your total score is: ${incorrectFixture.totalScore}`);
          cy.get('.feedback').should('contain', `You were correct!`);
          cy.get('.feedback').should(
            'contain',
            `The correct translation for ${languageHeadFixture.nextWord} was ${incorrectFixture.answer}!`
          );
          cy.get('button').should('have.text', `Try another word!`);
        });
      });
